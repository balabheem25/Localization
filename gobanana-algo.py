...
class ParticleFilter:
    ...
    def initialize_particles(self):
        #initializing particles in the circular space uniformly
        angles = np.random.uniform(0, 2 * np.pi, self.num_particles)
        radii = np.sqrt(np.random.uniform(0, self.radius**2, self.num_particles))
    ...
    def predict(self, noise_std=0.5):
        #applies no random motion or at rest which gives random noise or uncertainties to particles with given standard deviation
        #self.particles += np.random.normal(0, noise_std, self.particles.shape)
        #applies random motion to the particles with given standard deviation
        if motion_std 
